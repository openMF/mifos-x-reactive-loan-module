...
class LoanWithdrawnHandlerTest {
    ...
    @Test
    void handleCancelsAggregatorAndDeletesSnapshot() throws Exception {
        ...
        when(service.onLoanWithdrawn(loan)).thenReturn(Mono.empty());
        when(snapshotRepo.deleteByLoanId(1L)).thenReturn(Mono.empty());

        handler.handle(payload);

        verify(service).onLoanWithdrawn(loan);
