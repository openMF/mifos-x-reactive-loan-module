...
public class MessageConsumerServiceImpl implements MessageConsumerService {
    ...
    public Flux<EventMessageDTO> getMessages() {
        return repository.findAll().flatMap(this::toDto) // convert one by one
                .subscribeOn(Schedulers.boundedElastic());
    }

    @Override
    public Flux<EventMessageDTO> getMessagesByType(String eventType) {
        return repository.findByType(eventType).flatMap(this::toDto).subscribeOn(Schedulers.boundedElastic());
    }
    ...
    private Mono<EventMessageDTO> toDto(EventMessage msg) {
        return Mono.fromCallable(() -> convert(msg)).subscribeOn(Schedulers.boundedElastic());
    }
    ...
    private EventMessageDTO convert(EventMessage msg)
        ...
        Object payload = fromByteBuffer.invoke(null, byteBufferConvertor.convert(msg.getPayload()));

        /* BULK EVENTS */
        if ("BulkBusinessEvent".equalsIgnoreCase(msg.getType())) {
            Method datasMethod = payload.getClass().getMethod("getDatas");
            @SuppressWarnings("unchecked")
            List<BulkMessageItemV1> bulkItems = (List<BulkMessageItemV1>) datasMethod.invoke(payload);

            String aggregatedPayload = bulkItems.stream().map(this::safeBulkConvert).collect(Collectors.joining(System.lineSeparator()));
    ...
    private EventMessageDTO dtoOf(EventMessage src, String body) {
        return new EventMessageDTO(src.getEventId(), src.getType(), src.getCategory(), src.getTenantId(), iso(src.getCreatedAt()), body,
                src.getBusinessDate());
    }
